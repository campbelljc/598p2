...
def entropy(data):
    ...
    class_count = Counter(data) # ref : http://ludovf.net/blog/python-collections-defaultdict/

    probs = [float(class_count[c])/total_samples for c in class_count] # divide counts by len
...
def information_gain(data, feature_split_index):
    ...
    for partition in data_split_on_feature.values():
        ...
        prob = float(len(partition)) / total_samples
        # formula for cond. entropy: P(x = feature)*E(classes | x = feature)
        cond_entropy += prob * entropy(targets)
        
    targets = [x[-1] for x in data] # get classes for each item in this partition
    info_gain = entropy(targets) - cond_entropy
    return info_gain
...
def build_decision_tree(data, feature_indices):
    ...
    if len(feature_indices) == 0: # no features left...
        ...
        if len(set(classes)) == 1: # but all the data has the same class, so it doesn't matter.
            ...
            feature_indices.remove(best_feature) # remove the chosen feature from the array of features to look at
...
# tree: A tree built by build_decision_tree
# data: A list representing a data point, where each element represents a feature.
def predict_one(tree, data):

    # If the subtree isn't a dictionary, then we're at a root
    if not isinstance(tree, types.TupleType):
        return tree;
